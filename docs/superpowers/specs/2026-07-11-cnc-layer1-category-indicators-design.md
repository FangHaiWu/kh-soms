...
**8 category** trong `osint_keywords.category` (mở rộng từ 3 → 8 cho khớp thực tế nghiệp vụ CNC).
Bộ đầy đủ ~125 keyword ở file seed `backend/database/seeds/seed-osint-keywords-cnc.sql`:
- `lua-dao`: lừa đảo CĐTS — giả danh công an/VKS/tòa, việc nhẹ lương cao, CTV chốt đơn, sàn forex/BO, khóa sim, OTP, hack facebook...
- `mua-ban-dlcn`: bán/sỉ data, data khách hàng, data ngành, CCCD, lộ lọt dữ liệu, mua bán/thuê tài khoản ngân hàng...
- `tan-cong-ma-doc`: mã độc, ransomware, tống tiền dữ liệu, tấn công mạng, DDoS, lỗ hổng, phishing, spyware...
- `co-bac-ca-do`: cá độ bóng đá, nhà cái, tài xỉu, lô đề, casino online, game bài đổi thưởng...
- `tin-dung-den`: app vay, vay không thế chấp, tín dụng đen, vay nóng, bùng app, khủng bố đòi nợ...
- `deepfake-gia-mao`: deepfake, cắt ghép hình ảnh, giả mạo video, giả giọng nói...
- `rua-tien`: rửa tiền, tiền bẩn, trung gian thanh toán trái phép, đổi tiền ảo...
- `kich-dong-xuyen-tac`: (keyword YẾU — chủ yếu chờ LLM L3) kích động, xuyên tạc, phản động, tin giả...

Priority 1 (nóng) … 5 (yếu/cần ngữ cảnh). `scope='global'` mặc định. Seed additive
(`ON CONFLICT (category, keyword) DO NOTHING`) — không đụng keyword cũ, chạy lại an toàn.
